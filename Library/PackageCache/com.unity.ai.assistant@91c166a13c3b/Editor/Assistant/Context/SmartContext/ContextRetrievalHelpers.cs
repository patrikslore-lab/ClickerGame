...
namespace Unity.AI.Assistant.Editor.Context.SmartContext
{
    internal static class ContextRetrievalHelpers
    {
        ...
        {
            var res = FindAssetsWithFuzzyMatch(assetName, typeName, true);
            if (res?.Count() == null)
                return null;
            ...
            if (!first.IsDefault)
            {
                return AssetDatabase.LoadAssetAtPath<Object>(first.Path);
            }

            return null;
        }

        internal static IEnumerable<PathAndScore> FindAssetsWithFuzzyMatch(string assetName, string typeName, bool returnFirstMatch)
        {
            var filter = string.IsNullOrEmpty(typeName) ? "" : $"t:{typeName}";
            ...
            if (string.IsNullOrEmpty(assetName))
                return objectsToSearch.Select(e => new PathAndScore() { Path = e, Score = 100 } );

            return FuzzyObjectSearchByPathWithScore(assetName, objectsToSearch);
        }
